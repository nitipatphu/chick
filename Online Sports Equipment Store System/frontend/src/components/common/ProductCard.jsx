...
const ProductCard = ({ product }) => {
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-xl hover:border-sports-red/50 transition-all duration-300 group overflow-hidden">
      <Link to={`/product/${product.id}`}>
        <div className="relative aspect-[4/5] overflow-hidden bg-white/5">
          ...
          {(!user || user.role === 'customer') && (
            <button
              ...
              className="absolute bottom-4 right-4 w-11 h-11 bg-sports-red/80 backdrop-blur-md rounded-full flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition-all duration-300 hover:bg-red-600 shadow-lg hover:shadow-xl hover:scale-110"
        ...
        <div className="p-4 bg-transparent border-t border-white/5">
          <p className="text-gray-400 text-xs uppercase tracking-wider font-medium">{product.category || 'ทั่วไป'}</p>
          <h3 className="font-bold text-lg text-white hover:text-sports-red transition line-clamp-1 mt-1">{product.name}</h3>
