  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (modalMode === 'create') {
        ...
        const { stock, ...productDataWithoutStock } = formData;
        await inventoryService.updateProduct(selectedProduct.id, productDataWithoutStock);
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="border-t border-gray-800 pt-4">
                    ...
                    <div className="flex gap-2 mb-3">
                      <input
                        ...
                        placeholder="เพิ่มชื่อสีใหม่ เช่น ดำ, ไทเทเนียม..."
                        className="flex-1 px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-white placeholder-gray-500 focus:border-sports-red focus:outline-none transition"
                      />
                      <button
                        type="button"
                        onClick={() => {
                        ...
                        className="px-4 py-2 bg-white/10 hover:bg-sports-red border border-white/20 text-white rounded-lg transition"
                      >
                        เพิ่มสี
                      </button>
                    </div>
                    <div className="flex gap-3 flex-wrap">
                      {formData.colors.map(color => (
                        <div
                          key={color}
                          className="flex items-center gap-2 bg-white/10 px-3 py-1.5 rounded-full border border-white/20"
                        >
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              delete newColorImages[color];
                              setFormData({ ...formData, colors: newColors, colorImages: newColorImages });
                            }}
                    ...
                    {formData.colors.length > 0 && (
                      <p className="text-xs text-gray-400 mt-2">เพิ่มแล้ว {formData.colors.length} สี</p>
                    )}
                  </div>
                    ...
                    <div className="border-t border-gray-800 pt-4 space-y-3">
                      ...
                      {formData.colors.map(color => (
                        <div key={color} className="flex items-center gap-3">
                          <span className="text-sm font-medium text-white bg-white/10 px-2 py-1 rounded truncate min-w-[60px] text-center" title={color}>{color}</span>
                          <input
                            type="text"
                  ...
                  <div className="border-t border-gray-800 pt-4">
                    <div className="flex justify-between items-center mb-3">
                      <label className="text-gray-400 text-sm font-medium">ขนาดที่มีจำหน่าย</label>
                      <div className="flex gap-2">
                        <button type="button" onClick={() => setFormData({ ...formData, sizes: ['S', 'M', 'L', 'XL'] })} className="text-xs bg-white/5 hover:bg-white/10 px-2 py-1 rounded transition text-gray-300">เสื้อผ้า</button>
                        <button type="button" onClick={() => setFormData({ ...formData, sizes: ['38', '39', '40', '41', '42', '43', '44'] })} className="text-xs bg-white/5 hover:bg-white/10 px-2 py-1 rounded transition text-gray-300">รองเท้า</button>
                        <button type="button" onClick={() => setFormData({ ...formData, sizes: ['5', '6', '7'] })} className="text-xs bg-white/5 hover:bg-white/10 px-2 py-1 rounded transition text-gray-300">ลูกบอล</button>
                      </div>
                    </div>
                    <div className="flex gap-2 mb-3">
                      <input
                        ...
                        placeholder="เพิ่มขนาดใหม่ เช่น S, 10 US, Size 7..."
                        className="flex-1 px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-white placeholder-gray-500 focus:border-sports-red focus:outline-none transition"
                      />
                      <button
                        type="button"
                    ...
                    {formData.sizes.length > 0 && (
                      <p className="text-xs text-gray-400 mt-2">เพิ่มแล้ว {formData.sizes.length} ขนาด</p>
                    )}
                  </div>
