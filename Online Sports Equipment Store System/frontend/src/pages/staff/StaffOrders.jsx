// frontend/src/pages/staff/StaffOrders.jsx
...
export default function StaffOrders() {
  ...
  async function loadOrders() {
    try {
      setLoading(true);
      setError('');
      const data = await orderService.getStaffOrders();
  ...
  const getStatusChangeRestrictionMessage = (order) => {
    if (!order) return '';
    ...
    if (!order.slipImage) {
      return '⚠️ ลูกค้ายังไม่ได้อัปโหลดสลิปการโอนเงิน กรุณารอลูกค้าอัปโหลดสลิป';
    }

    if (order.slipImage && !order.paymentConfirmed) {
      return '⚠️ รอผู้ดูแลระบบ (Admin) ยืนยันสลิปการโอนเงินก่อน จึงจะสามารถเปลี่ยนสถานะได้';
    }
  ...
  const getStatusButtonState = (order) => {
    if (!order) return { disabled: true, message: '' };

    if (order.status === 'cancelled' || order.status === 'delivered') {
      return { disabled: true, message: 'ไม่สามารถเปลี่ยนแปลงสถานะนี้ได้' };
    }
    ...
    if (!order.slipImage) {
      return { disabled: true, message: 'รอสลิปการโอนเงิน' };
    }

    if (order.slipImage && !order.paymentConfirmed) {
      return { disabled: true, message: 'รอ Admin ยืนยันสลิป' };
    }
    ...
    return { disabled: true, message: 'ไม่สามารถดำเนินการได้' };
  };
  ...
  async function handleStatusChangeSubmit(e) {
    ...
    if (!canChangeStatus(selectedOrder)) {
      const restrictionMsg = getStatusChangeRestrictionMessage(selectedOrder);
      await Swal.fire({
        title: '⚠️ ไม่สามารถเปลี่ยนสถานะได้',
        text: restrictionMsg || 'กรุณาตรวจสอบเงื่อนไขการเปลี่ยนสถานะ',
        icon: 'warning',
    ...
    try {
      ...
      if (newStatus === 'shipped') {
        if (!trackingCourier || !trackingNumber) {
          await Swal.fire('⚠️ ข้อมูลไม่ครบ', 'กรุณาระบุบริษัทขนส่งและหมายเลขพัสดุ', 'warning');
          setUpdatingStatus(false);
          return;
        }
        extraData.trackingCourier = trackingCourier;
        extraData.trackingNumber = trackingNumber;
      }

      const res = await orderService.updateOrderStatus(selectedOrder.id, newStatus, extraData);
      if (res.success || res.order) {
        setOrders(orders.map(o => (o.id === selectedOrder.id ? { 
          ...o, 
          status: newStatus,
          trackingCourier: newStatus === 'shipped' ? trackingCourier : o.trackingCourier,
          trackingNumber: newStatus === 'shipped' ? trackingNumber : o.trackingNumber
        } : o)));
        setSelectedOrder({ 
          ...selectedOrder, 
          status: newStatus,
          trackingCourier: newStatus === 'shipped' ? trackingCourier : selectedOrder.trackingCourier,
  ...
  const pendingSlipConfirmation = orders.filter(o =>
    o.slipImage && !o.paymentConfirmed && o.status !== 'cancelled'
  ).length;
    ...
    <div className="min-h-screen bg-sports-dark text-white pt-20 pb-20">
      <div className="container max-w-7xl mx-auto px-4">
        <div className="flex items-center justify-between mb-4">
          <Link to="/staff" className="text-gray-400 hover:text-white inline-flex items-center gap-1 transition">
            &larr; กลับแดชบอร์ด
          </Link>
          <span className="text-xs text-gray-500 bg-white/5 px-3 py-1 rounded-full">
            สิทธิ์: พนักงานจัดการคำสั่งซื้อ
          </span>
        </div>
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-400 text-sm mt-1">
            ...
            {pendingSlipConfirmation > 0 && (
              <span className="text-yellow-500 ml-2">
                ⚠️ รอ Admin ยืนยันสลิป {pendingSlipConfirmation} รายการ
              </span>
            )}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4 mb-8">
          ...
          <div className="glass-card p-5 text-center border-orange-500/30">
            <p className="text-orange-500 text-xs font-semibold uppercase tracking-wider">รอ Admin ยืนยันสลิป</p>
            <p className="text-2xl font-black mt-2 text-orange-500">{pendingSlipConfirmation} รายการ</p>
          </div>
        </div>
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-white/5">
                {sortedOrders.length === 0 ? (
                  ...
                  const buttonState = getStatusButtonState(order);
                  return (
                        ...
                        {order.slipImage ? (
                          <div className="flex flex-col items-center gap-1">
                            ...
                            {order.paymentConfirmed ? (
                              ...
                              <span className="text-xs text-yellow-500 bg-yellow-500/20 px-2 py-0.5 rounded-full">รอ Admin ตรวจสอบ</span>
                            )}
                          </div>
                        ) : (
              ...
              <div className="space-y-6">
                <div className="space-y-3">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    {(selectedOrder.trackingCourier || selectedOrder.trackingNumber) && (
                      <div className="bg-blue-500/10 p-3 rounded-lg border border-blue-500/20 mt-2">
                        <p className="text-blue-400 text-xs mb-1 font-bold">📦 ข้อมูลการจัดส่ง:</p>
                        <div className="text-white text-xs space-y-1">
                          <p><span className="text-gray-400">ขนส่ง:</span> {selectedOrder.trackingCourier || '-'}</p>
                          <p><span className="text-gray-400">เลขพัสดุ:</span> <span className="font-mono font-bold text-sports-red">{selectedOrder.trackingNumber || '-'}</span></p>
                        </div>
                      </div>
                    )}
                      ...
                      <div className="bg-white/5 p-3 rounded-lg border border-white/5">
                        ...
                        <div className="flex items-center justify-center gap-2 mt-2">
                          {selectedOrder.paymentConfirmed ? (
                            <span className="text-green-400 text-sm flex items-center gap-1">✅ Admin ยืนยันการชำระเงินแล้ว</span>
                          ) : (
                            <span className="text-yellow-500 text-sm flex items-center gap-1">⏳ รอ Admin ยืนยันสลิป</span>
                          )}
                        </div>
                      </div>
                    )}
                ...
                <div className="space-y-3">
                  ...
                  <div className="bg-blue-500/10 p-3 rounded-lg border border-blue-500/20">
                    <p className="text-blue-400 text-xs flex items-center gap-2">
                      <span>ℹ️</span>
                      {selectedOrder.paymentMethod === 'cod' || selectedOrder.paymentMethod === 'cash_on_delivery' ? (
                        'สามารถเปลี่ยนสถานะได้ทันที (COD)'
                      ) : selectedOrder.slipImage && selectedOrder.paymentConfirmed ? (
                        '✅ Admin ยืนยันสลิปแล้ว สามารถเปลี่ยนสถานะได้'
                      ) : selectedOrder.slipImage && !selectedOrder.paymentConfirmed ? (
                        '⏳ รอ Admin ยืนยันสลิปก่อน จึงจะเปลี่ยนสถานะได้'
                      ) : (
                        '📤 รอลูกค้าอัปโหลดสลิปการโอนเงิน'
                      )}
                    </p>
                  </div>
                  ...
                  <form onSubmit={handleStatusChangeSubmit} className="space-y-4">
                    ...
                    <div className="space-y-2">
                      <label htmlFor="staff-order-status-select" className="text-xs text-gray-400 block">เปลี่ยนสถานะเป็น:</label>
                      <select
                        id="staff-order-status-select"
                        value={newStatus}
                        onChange={e => setNewStatus(e.target.value)}
                        disabled={!canChangeStatus(selectedOrder)}
                        className={`w-full border rounded-lg text-white py-2.5 px-3 focus:outline-none text-sm ${!canChangeStatus(selectedOrder)
                            ? 'bg-gray-700/50 border-gray-600 text-gray-400 cursor-not-allowed'
                            : 'bg-white/5 border-white/10 focus:border-sports-red'
                          }`}
                      >
                      ...
                      {!canChangeStatus(selectedOrder) && (
                        <p className="text-yellow-500 text-xs mt-1">
                          {getStatusChangeRestrictionMessage(selectedOrder)}
                        </p>
                      )}
                      {newStatus === 'shipped' && (
                    ...
                    {(() => {
                      ...
                      const isButtonDisabled = updatingStatus || (!canChangeStatus(selectedOrder)) || (selectedOrder.status === newStatus && !isTrackingModified);
                      return (
                        <button
                          type="submit"
